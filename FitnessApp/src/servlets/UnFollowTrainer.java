...
@MultipartConfig
@WebServlet("/UnFollowTrainer")
public class UnFollowTrainer extends HttpServlet {
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(userDetails!=null){
				...
				DAO dao = new DAO();
				boolean result = dao.deleteFollowTrainer(follow);
				if (result == true) {
					session.setAttribute("message", "Successfully UnFollowed!");
					response.sendRedirect("UserHome.jsp");
				} else {
					session.setAttribute("message", "You are already Unfollowed a trainer!");
					response.sendRedirect("UserHome.jsp");
				}
			} else {
